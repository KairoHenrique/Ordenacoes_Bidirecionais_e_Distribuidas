...
class QueueArrayPigeonholeSort
  ...
  def pigeonhole_sort
    holes = Array.new(10) { [] }
    @queue.each { |r| holes[(r.rating * 2).round - 1] << r }
    @queue = holes.flatten
  end
end

if ARGV.size < 2
  puts "Uso: ruby queue_array_pigeonhole_sort.rb <arquivo.csv> <N>"
  exit
end

path = ARGV[0]
n = ARGV[1].to_i
fila = QueueArrayPigeonholeSort.new(n)
