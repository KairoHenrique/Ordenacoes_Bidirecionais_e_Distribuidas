...
class QueuePointerPigeonholeSort
  ...
  def pigeonhole_sort
    ...
    cur = @head
    while cur
      idx = (cur.rating.rating * 2).round - 1
      holes[idx] << cur
      cur = cur.next
    end
    @head = @tail = nil
...
if ARGV.size < 2
  puts "Uso: ruby queue_pointer_pigeonhole_sort.rb <arquivo.csv> <N>"
  exit
end

path = ARGV[0]
n = ARGV[1].to_i
fila = QueuePointerPigeonholeSort.new
...
time = Benchmark.realtime { fila.pigeonhole_sort }
...
puts "Quantidade de registros: #{fila.size}"