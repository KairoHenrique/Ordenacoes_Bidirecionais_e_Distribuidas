...
class ListArrayPigeonholeSort
  ...
  def pigeonhole_sort
    holes = Array.new(10) { [] }
    @data.each { |r| holes[(r.rating * 2).round - 1] << r }
    @data = holes.flatten
  end
end

if ARGV.size < 2
  puts "Uso: ruby list_array_pigeonhole_sort.rb <arquivo.csv> <N>"
  exit
end

path = ARGV[0]
n = ARGV[1].to_i
lista = ListArrayPigeonholeSort.new(n)
