...
public class QueuePointerPigeonholeSort
{
    ...
    void pigeonholeSort()
    {
        ...
        for(Node p=head;p!=null;p=p.nx) holes[(int)Math.round(p.v.r*2)-1].addLast(p);
    ...
    throws Exception
    {
        if(a.length<2)
        {
            System.out.println("Uso: java QueuePointerPigeonholeSort <ratings.csv> <N>");
            return;
        }

        int N=Integer.parseInt(a[1]); QueuePointerPigeonholeSort Q=new QueuePointerPigeonholeSort();
        ...
        System.out.printf("fila ponteiro %d regs %.2f ms%n",N,(t1-t0)/1e6);
        Node p=Q.head;for(int i=0;i<20&&p!=null;i++,p=p.nx)System.out.println(p.v);
    }
}
